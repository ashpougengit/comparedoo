...
if (process.env.NODE_ENV === 'production') {
  ...
  // Ensure the database exists in /tmp (Vercel's writable directory)
  if (!fs.existsSync(destPath)) {
    const sourcePath = path.join(process.cwd(), 'comparedoo.db'); // Database in project root

    // Check if the source file exists
    if (fs.existsSync(sourcePath)) {
      console.log("Source DB file exists at:", sourcePath);
      console.log("Copying database file to /tmp directory...");

      // Copy database file from project root to /tmp
      fs.copyFileSync(sourcePath, destPath);
      console.log("Database file successfully copied to:", destPath);

      // Check file size of source and destination
      const sourceStats = fs.statSync(sourcePath);
      const destStats = fs.statSync(destPath);
      console.log(`Source DB file size: ${sourceStats.size} bytes`);
      console.log(`Destination DB file size: ${destStats.size} bytes`);
    } else {
      console.error("Source DB file does not exist at:", sourcePath);
    }
  } else {
    console.log("Database already exists at /tmp/comparedoo.db");
  }
...
// Log whether the database exists
...
// Fetch country general info
export function fetchCountryGeneralInfo(country) {
  if (!checkTableExists('country_general_info')) {
    throw new Error('Table country_general_info does not exist');
  }
