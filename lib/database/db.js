...
const pool = new Pool({
    ...
    ssl: {
        rejectUnauthorized: true,     // Disable SSL certificate validation (use this if you don't have a custom SSL cert)
      }
});
...
export async function fetchCountryGeneralInfo(country) {
    try {
        ...
        const res = await client.query('SELECT * FROM country_general_info WHERE country = $1', [country]);
...
export async function fetchCountryCostInfo(country) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT * FROM country_cost_info WHERE country = $1', [country]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchCountryStandardInfo(country) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT * FROM country_standard_info WHERE country = $1', [country]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchUSStateGeneralInfo(state) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT * FROM us_state_general_info WHERE state = $1', [state]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchUSStateCostInfo(state) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT * FROM us_state_cost_info WHERE state = $1', [state]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchUSStateStandardInfo(state) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT * FROM us_state_standard_info WHERE state = $1', [state]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchCurrencyInfo(country) {
    try {
        const client = await pool.connect();  // Get a connection from the pool
        const res = await client.query('SELECT currencyName, currencySymbol, unitValueInUSD FROM country_general_info WHERE country = $1', [country]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchHealthExpenditurePercentage(country) {
    try {
        const client = await pool.connect(); // Get a connection from the pool
        const res = await client.query('SELECT currentHealthExpenditurePercentage FROM country_standard_info WHERE country = $1', [country]);
        client.release();  // Release the connection back to the pool
        return res.rows.length > 0 ? res.rows[0] : null;
    } catch (error) {
...
export async function fetchPropertyAndIncomeTaxInfo(locationType, entity) {
    try {
        ...
        const query = `SELECT incomeTaxRates, propertyTaxRates FROM ${tableName} WHERE ${locationType} = $1`;
