...
// const dbPath = process.env.NODE_ENV === 'production' ? '/tmp/comparedoo.db' : 'comparedoo.db';

// Check if running in production (Vercel)
let dbPath = 'comparedoo.db'; // Default path for local dev

// Define the source and destination paths
const sourcePath = path.join(__dirname, 'comparedoo.db');
...
if (process.env.NODE_ENV === 'production') {
  const tmpDbPath = '/tmp/comparedoo.db';

  // Copy database to /tmp if it's not already there
  if (!fs.existsSync(tmpDbPath)) {
    const sourceDbPath = path.resolve(__dirname, 'comparedoo.db');
    fs.copyFileSync(sourceDbPath, tmpDbPath);
  }

  dbPath = tmpDbPath; // Update path to /tmp
}
