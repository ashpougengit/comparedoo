...
if (process.env.NODE_ENV === 'production') {
  const sourcePath = path.join(process.cwd(), 'public', 'comparedoo.db');
  const destPath = '/tmp/comparedoo.db';

  // Ensure the database is copied to /tmp if it doesn't already exist
  ...
  dbPath = destPath; // Update the path for production
}
