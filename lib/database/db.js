...
let dbPath = path.join(__dirname, 'public/comparedoo.db'); // Default path for both local dev and production

// Define the destination path for Vercel’s /tmp directory
const tmpDbPath = '/tmp/comparedoo.db';


// Check if running in production (Vercel)
if (process.env.NODE_ENV === 'production') {
  // Copy the database to the /tmp directory only if it doesn't exist there
  if (!fs.existsSync(tmpDbPath)) {
    fs.copyFileSync(dbPath, tmpDbPath);
  }
  dbPath = tmpDbPath; // Use the /tmp path in production
}
