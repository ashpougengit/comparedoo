import { renderTimezones } from "../date-and-time/dateAndTime";
...
export const titleCased = function (str) {
  ...
  return decodedStr
    ...
    .split(' ')
    .map((word, i) =>
      exceptions.includes(word) && i !== 0 ? word : word.charAt(0).toUpperCase() + word.slice(1)
    )
    .join(' ');
};
...
export const Capitalize = (str) => {
  ...
  const titleCaseArray = strArray.map(word => word[0].toUpperCase() + word.slice(1).toLowerCase())
...
export function removeZerosAfterDecimal(num) {
  let numberString = num.toFixed(2);
...
export function formatNumberWithCommas(value) {
  if (typeof value === 'string') {
    if (value.includes('<br />')) {
      const valueArr = value.split('<br />');
      return valueArr
      // return valueArr.map((val, index) => (
      //   <span key={index}>
      //     {val.trim()} {/* Render the value */}
      //     {index < valueArr.length - 1 && <br />} {/* Insert <br /> between values */}
      //   </span>
      // ));
    }
    return value;
  ...
  return removeZerosAfterDecimal(value); // Handle number formatting here
}
...
export function formatPercentage(value) {
  if (!value || ['yet to update', 'n/a'].includes(value.toLowerCase())) return 'Yet to Update';

  return value.includes(' to ')
    ? value.split(' to ').map(rate => `${rate}%`).join(' to ')
    : `${value}%`;
}
