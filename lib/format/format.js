import { renderLineBreak } from "../date-and-time/dateAndTime";
...
export const camelToTitleCase = (word, isCountry = null) => {
  ...
  // List of exceptions that should remain lowercase unless they're the first word
  const exceptions = ['a', 'and', 'the', 'an', 'of', 'on', 'in', 'is'];
  ...
  // Split into words and handle exceptions
  const words = titleCasedWord.split(' ').map((w, index) => {
    if (index === 0) {
      return w.charAt(0).toUpperCase() + w.slice(1); // Always capitalize the first word
    }
    return exceptions.includes(w.toLowerCase()) ? w.toLowerCase() : w.charAt(0).toUpperCase() + w.slice(1);
  });
...
export function formatNumberWithCommas(value) {
  if (typeof value === 'string') {
    if (value.includes('<br />')) {
      return renderLineBreak(value.split('<br />'))
