...
function NetworkAndCommunication({
  ...
  const state1MobilePhonePlanCostPerMonth =
    state1CostInfo.mobilePhonePlanCostPerMonth;
  const state1LandlinePhoneServicePerMonth =
    state1CostInfo.landlinePhoneServicePerMonth;
  const state1CableTVServicePerMonth = state1CostInfo.cableTVServicePerMonth;
  const state1TotalCost =
    (state1WifiCostPerMonth || 0) +
    (state1MobilePhonePlanCostPerMonth || 0) +
    (state1LandlinePhoneServicePerMonth || 0) +
    (state1CableTVServicePerMonth || 0);

  const state2 = state2CostInfo.state;
  const state2WifiCostPerMonth = state2CostInfo.wifiCostPerMonth;
  const state2MobilePhonePlanCostPerMonth =
    state2CostInfo.mobilePhonePlanCostPerMonth;
  const state2LandlinePhoneServicePerMonth =
    state2CostInfo.landlinePhoneServicePerMonth;
  const state2CableTVServicePerMonth = state2CostInfo.cableTVServicePerMonth;
  const state2TotalCost =
    (state2WifiCostPerMonth || 0) +
    (state2MobilePhonePlanCostPerMonth || 0) +
    (state2LandlinePhoneServicePerMonth || 0) +
    (state2CableTVServicePerMonth || 0);
